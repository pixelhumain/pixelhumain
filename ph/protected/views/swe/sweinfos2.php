...
.control-label{width:250px;float:left;}
appContent div.controls {float:left;margin-left:0px;}
.form-horizontal .control-group {   margin-bottom:5px;}
.control-groupL textarea{width:350px;height:100px;}
</style>
...
<div class="container appContent">
    ...
    <div class="hero-unit">
    	<h2> Fiche Feedback </h2>
    	<h3> Merci de partager vos pensées. </h3>
        
        <form id="sweFeedBackForm" class="form-horizontal" >
            ...
            <div class="control-group">
              <label class="control-label" for="pitcher">Avez-vous pitché ?</label>
             <input type="checkbox" name="pitcher" id="pitcher" <?php if(isset($me["pitcher"]))echo "checked"?>/></label>
            </div>
            ...
            <div class="control-group">
              <label class="control-label" for="contactMembresProlonge">Allez vous rester en contact avec les membres de votre équipe ?</label>
              <input type="checkbox" name="contactMembresProlonge" id="contactMembresProlonge" value="<?php if(isset($me["contactMembresProlonge"]))echo "checked"?>"/>
            </div>
            
            <!-- Multiple Radios -->
            <div class="control-group">
              <label class="control-label" for="autreContactNoue">Avez vous noué d'autres contacts ?</label>
              <input type="checkbox" name="autreContactNoue" id="autreContactNoue" value="<?php if(isset($me["autreContactNoue"]))echo "checked"?>"/>
            </div>
            ...
              <label class="control-label" for="temoignage">Merci par avance pour vos témoignages :</label>
              
                <textarea id="temoignage" name="temoignage" placeholder="dites ce que vous retenez, ce que cela vous a apporté ...(5 lignes max)">
            	<?php if(isset($me["temoignage"]))echo $me["temoignage"]?>
            	</textarea>
            </div>
        	 <div class="clear"></div>
        </form>
		...
		<div class="modal-footer pull-left">
            <button class="btn btn-primary" id="submitsweFeedBackForm" onclick="$('#sweFeedBackForm').submit();">Enregistrer</button>
          </div>
    </div>
</div>
...
initT['sweGraphInit'] = function(){
    ...
    $("#sweFeedBackForm").submit( function(event){
    	if($('.error').length){
    		alert('Veuillez remplir les champs obligatoires.');
    	}else{
        	event.preventDefault();
        	$("#sweFeedBackForm").modal('hide');
        	NProgress.start();
        	$.ajax({
        	  type: "POST",
