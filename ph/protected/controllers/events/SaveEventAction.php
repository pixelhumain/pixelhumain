...
class SaveEventAction extends CAction
{
    public function run()
    {
			...
			if($user["isOrganiser"] == 1){
				$created_events = $user["createEvents"];
				$created_events_array = explode(",",$created_events);
				$filtered_events = array_filter($created_events_array);
				$ID = (string)$newInfos["_id"];
				array_push($filtered_events,$ID);
				$events_array["createEvents"] = implode(",",$filtered_events);
				PHDB::update(PHType::TYPE_CITOYEN,array("_id"=> new MongoId($userId)), array('$set' => $events_array));
			}
	  ...
	  private function processImage($image, $eventID) {
	  	$image_name							= "image_".$eventID;
