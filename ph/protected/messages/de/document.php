<?php

	return array(
		"only" => "Nur",
		"Document has been deleted" => "Das Dokument wurde gelöscht",
		"files are allowed!" => "sind erlaubt",
		"Error! Wrong HTTP method!" => "Fehler! Falsche HTTP Anwendung",
		"Something went wrong with your upload!" => "Das Laden des Dokuments ist fehlgeschlagen",
		"Document deleted" => "Das Dokument wurde gelöscht",
		"Document saved successfully" => "Das Dokument wurde gespeichert",
		"Image deleted" => "Das Bild wurde gelöscht",
		"You have no rights upload document on this item, you may only write a message !" => "Sie haben keine Rechte dazu, hier ein Bild hochzuladen. Sie dürfen nur eine Nachricht schreiben!",
		"You are not allowed to modify the document of this item !" => "Sie habe keine Rechte dazu ein Dokument hier zu verändern oder hochzuladen",
		"Collection of bookmarks<br>To classified favorite urls<br>And share it to the community"=>"Sammlung von Lesezeichen<br>Um Ihre URL Favoriten zu sortieren<br>Und um diese gemeinschaftlich zu teilen",
		"Collection of files<br>To download and share documents<br>With the community"=>"Dateisammlung<br>Zum Mitteilen und Herunterladen von Dokumenten<br>Mit allen",
	)

?>
